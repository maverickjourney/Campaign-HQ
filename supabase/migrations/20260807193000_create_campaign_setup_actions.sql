-- ============================================================
-- CAMPAIGN SEAT
-- PROTECTED CAMPAIGN SETUP SAVE + ACTIVATION ACTIONS
--
-- Browser clients never receive direct write access to the
-- onboarding table. Sensitive Setup writes happen only through
-- authenticated, AAL2-protected leadership RPCs.
-- ============================================================
...
begin
  ...
  -- ----------------------------------------------------------
  -- Track only steps proven complete by the submitted data.
  -- ----------------------------------------------------------
  ...
  where
    ...
    and step_key =
      'branding'
    and normalized_active_theme in (
      'red',
      'blue',
      'purple',
      'neutral',
  ...
  where
    ...
    and step_key =
      'campaign_identity'
    and normalized_public_name <> ''
    and normalized_campaign_type <> ''
    and normalized_office_sought
      is not null
    and normalized_jurisdiction_name
      is not null;
  ...
  if (
    normalized_primary_election_date
      is not null
    or normalized_general_election_date
      is not null
  ) then
    update
    public.workspace_onboarding_steps
...
is
  'AAL2-protected Campaign Seat leadership action that saves an onboarding draft without activating the workspace.';


-- ============================================================
-- ACTIVATE CORE CAMPAIGN WORKSPACE
-- ============================================================
...
begin
  ...
  if not actor_is_authorized then
    raise exception
      'Your campaign role is not authorized to activate Campaign Seat.'
      using errcode = '42501';
  end if;


  -- ----------------------------------------------------------
  -- Final activation validations.
  -- Draft saving allows later sections to remain incomplete.
  -- Activation does not.
  -- ----------------------------------------------------------
  ...
  if nullif(
    ...
    raise exception
      'Enter the office, race or public purpose before activation.';
  end if;
  ...
  if nullif(
    ...
    raise exception
      'Enter the campaign jurisdiction before activation.';
  end if;
  ...
  if (
    ...
    raise exception
      'Enter at least one election date before activation.';
  end if;
  ...
  if nullif(
    ...
    raise exception
      'Choose the campaign timezone before activation.';
  end if;
  ...
  set
    ...
    setup_metadata =
      setup_metadata ||
      jsonb_build_object(
        'core_workspace_activated_at',
        now(),
        'core_workspace_activated_by',
        actor_user_id,
        'next_setup_phase',
        'security'
      )
...
is
  'AAL2-protected Campaign Seat leadership action that activates the core workspace and advances onboarding to Security.';
