-- CAMPAIGN HQ — ACCESSIBLE MANUAL ROUTE ORDERING
-- Keeps local migration history aligned with the repaired production RPC.
...
begin
  ...
  select
    ...
    coalesce(
      max(stop.stop_order),
      0
    ) + requested_stop_count + 1000
    into
      route_stop_count,
      temporary_order_base
  ...
  select count(distinct submitted.stop_id)
  ...
  set stop_order =
    temporary_order_base + submitted.ordinality::integer
...
is
  'Leadership-only atomic route sequencing with positive temporary stop orders.';
