...
-- ============================================================
-- CAMPAIGN SEAT
-- FINAL CLIENT ACTIVATION
--
-- HARD RULES:
--
-- 1. AAL2 is required.
-- 2. Review must be complete.
-- 3. Billing provider must be real and subscription must be
--    trial or active.
-- 4. Every onboarding-selected OAuth provider must actually
--    be connected.
-- 5. Workspace creation is atomic.
-- 6. Product Account is explicitly bound to the Workspace.
-- 7. Campaign Owner membership is created automatically.
-- 8. Entitlements become workspace enabled_modules.
-- 9. Planned team invitations are prepared securely.
-- 10. Plain invitation tokens are returned once and only hashes
--     remain in the database.
--
-- This migration itself DOES NOT activate any customer.
-- ============================================================
...
declare
  ...
  selected_connection_count integer :=
    0;
...
begin
  ...
  public.seat_onboarding_run_steps
    as review_step
    on review_step.onboarding_run_id =
      onboarding.id
    and review_step.step_key =
      'review'

  left join
  public.seat_onboarding_run_steps
    as activation_step
    on activation_step.onboarding_run_id =
      onboarding.id
    and activation_step.step_key =
      'activation'
  ...
  review_complete :=
    activation_record.review_status =
      'complete';


  billing_ready :=
    activation_record.billing_provider in (
      'stripe',
      'manual'
    )

    and activation_record.subscription_status in (
      'trial',
      'active'
    );
    ...
    activation_blockers :=
      ...
      jsonb_build_array(
        jsonb_build_object(
          ...
          'title',
          'Onboarding Review must be confirmed',

          'description',
          'Confirm the final onboarding Review before Activation.'
        )
      );
  end if;
    ...
    activation_blockers :=
      ...
      jsonb_build_array(
        jsonb_build_object(
          ...
          'description',
          'Connect the Campaign Seat billing provider and establish an active or trial subscription before launch.'
        )
      );
  end if;
    ...
    activation_blockers :=
      ...
      jsonb_build_array(
        jsonb_build_object(
          ...
          'description',
          pending_connection_count::text ||
          ' selected OAuth provider connection(s) are still pending.'
        )
      );
  end if;
  ...
  return jsonb_build_object(
    ...
    'customer_name',
    activation_record.customer_name,

    'primary_contact',
    jsonb_build_object(
      'full_name',
      activation_record.full_name,

      'email',
      activation_record.email
    ),

    'account_status',
    activation_record.account_status,

    'onboarding_status',
    activation_record.onboarding_status,

    'onboarding_run_status',
    activation_record.onboarding_run_status,

    'current_step_key',
    activation_record.current_step_key,

    'activation_step_status',
    activation_record.activation_status,

    'billing',
    jsonb_build_object(
      'provider',
      activation_record.billing_provider,

      'status',
      activation_record.subscription_status,

      'currency',
      activation_record.currency,

      'monthly_amount_cents',
      activation_record.monthly_amount_cents,

      'onboarding_fee_cents',
      activation_record.onboarding_fee_cents,
    ...
    'activated',
    existing_workspace_id is not null,
