-- Campaign Seat
--
-- Keep onboarding audit timestamps semantic.
--
-- 1. Repeated draft progression no longer refreshes updated_at
--    for Branding, Campaign Identity, or Election Details when
--    each step is already complete with completion audit data.
--
-- 2. Workspace activation updates onboarding rows only for:
--      Review   -> complete
--      Security -> in_progress
--
-- No existing workspace or onboarding row is modified merely by
-- applying this migration. Only the two RPC definitions change.

CREATE OR REPLACE FUNCTION public.save_campaign_setup_draft(target_workspace_id uuid, target_payload jsonb, target_current_step text DEFAULT 'campaign_identity'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.activate_campaign_setup(target_workspace_id uuid, target_payload jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'private', 'pg_temp'
