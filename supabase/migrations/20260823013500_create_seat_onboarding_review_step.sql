...
--
-- Review confirms the saved onboarding configuration.
--
-- IMPORTANT:
--   * no workspace is created
--   * no invitation is sent
--   * no payment is processed
--   * no integration is marked connected
--
-- Review may advance to Activation even when Activation still
-- has real outstanding prerequisites.
-- ============================================================
...
begin
  ...
  where connection.product_account_id =
...
create or replace function
public.complete_my_seat_onboarding_review(
...
begin
  ...
  if not coalesce(
    ...
    raise exception
      'Confirm the onboarding details before continuing.';
  end if;
  ...
  then
    raise exception
      'Review is not the current onboarding step.';
  end if;
  ...
  if incomplete_step is not null then
    raise exception
      'Complete % before confirming Review.',
      incomplete_step;
  end if;
  ...
  billing_ready :=
    ...
    and onboarding_record.subscription_status <>
      'pending_billing';


  select count(*)

  into pending_connections
  ...
  values (
    actor_user_id,
    'seat_onboarding_review_confirmed',
    'notice',
